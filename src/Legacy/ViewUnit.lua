...
function ViewUnit.updateDisplay(unit)
    ...
    delay(4.1, function()
        if tick() - unit.lastDrawTime > 3 then
            model.Parent = nil
        end
    end)
    ...
    elseif unit.State == Unit.UnitState.COMBAT and (unit.Attack and unit.Attack ~= "") then

        local astr 

        if unit.Attack and unit.Attack ~= "" then
            astr = Util.positionStringToVector(unit.Attack)
        elseif unit.AttackUnit and unit.AttackUnit ~= "" then
            local ownerId, unitId = unpack(string.split(unit.AttackUnit, ':'))
            local hostile = currentWorld.Units[unitId]
            print(ownerId, unitId, hostile)
            if hostile then
                astr = Util.positionStringToVector(hostile.Position)
            end
        end
